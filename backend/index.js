...
app.get('/api/verifySecond', async (req, res) => {
    try {
      ...
      if (!accessToken && !refreshToken) {
        return res.status(401).json({ message: 'No hay sesión activa, inicie sesión nuevamente' });
      }
      ...
      if (!accessToken || error ) {
        console.log('El token de acceso es inválido o ha expirado. Intentando refrescar...');
  
        if (!refreshToken) {
          return res.status(401).json({ message: 'Token de refresco no encontrado. Inicie sesión nuevamente.' });
        }
